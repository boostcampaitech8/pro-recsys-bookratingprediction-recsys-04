...
def train_catboost(args, model, data, logger, setting):
    ...
    k_folds = 1
    if (
        hasattr(args, "model_args")
        and args.model in args.model_args
        and "use_optuna" in args.model_args[args.model]
    ):
        use_optuna = bool(args.model_args[args.model]["use_optuna"])
    if (
        hasattr(args, "model_args")
        and args.model in args.model_args
        and "optuna_n_trials" in args.model_args[args.model]
    ):
        n_trials = int(args.model_args[args.model]["optuna_n_trials"])
    if (
        hasattr(args, "model_args")
        and args.model in args.model_args
        and "k_folds" in args.model_args[args.model]
    ):
    ...
    if use_optuna or k_folds > 1:
        ...
        def make_params(trial=None):
            if trial is None:
                p = dict(
                    ...
                    learning_rate=getattr(
                        args.model_args[args.model], "learning_rate", 0.03
                    ),
                    ...
                    task_type=(
                        "GPU"
                        if getattr(args.model_args[args.model], "task_type", "CPU")
                        == "GPU"
            ...
            p = dict(
                ...
                task_type=(
                    "GPU"
                    if getattr(args.model_args[args.model], "task_type", "CPU") == "GPU"
                    and torch.cuda.is_available()
                    else "CPU"
                ),
                devices=getattr(args.model_args[args.model], "devices", "0"),
            ...
            if hasattr(model, "fold_models"):
                for idx, fm in enumerate(model.fold_models):
                    model_path = os.path.join(
                        args.train.ckpt_dir,
                        f"{setting.save_time}_{args.model}_fold{idx}{ext}",
                    )
                    fm.save_model(model_path)
            else:
                model_path = os.path.join(
                    args.train.ckpt_dir, f"{setting.save_time}_{args.model}_best{ext}"
                )
