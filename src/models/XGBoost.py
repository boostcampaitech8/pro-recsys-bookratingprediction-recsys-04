...
class XGBoost(nn.Module):
    def __init__(self, args, data, global_seed=None):
        ...
        random_seed = (
            ...
            else (getattr(args, "random_seed", None) or 42)
        )
        use_gpu = (
            getattr(args, "task_type", "CPU") == "GPU" and torch.cuda.is_available()
        )
    ...
    def fit(self, X, y, eval_set=None):
        ...
        self.model.fit(
            X, y, eval_set=evals, verbose=getattr(self.args, "verbose", False)
        )
        self.is_fitted = True
        return self
    ...
    def forward(self, x):
        ...
        return torch.tensor(
            preds,
            dtype=torch.float32,
            device="cuda" if (isinstance(x, torch.Tensor) and x.is_cuda) else "cpu",
        )
