...
def process_img_data(books, args):
    ...
    """
    books_ = books.copy()
    books_["img_path"] = books_["img_path"].apply(
        lambda x: f"{args.dataset.data_path}{x}"
    )
    img_vecs = []
...
def image_data_load(args):
    ...
    for col in sparse_cols:
        ...
        # 2. [수정됨] 사전(label2idx)을 이용해 Train/Test 일관성 있게 매핑
        # merge 과정에서 생길 수 있는 결측치를 위해 fillna('unknown') 필수
...
def image_data_loader(args, data):
    ...
    test_dataset = Image_Dataset(
        data["test"][data["field_names"]].values, data["test"]["img_vector"].values
    )
