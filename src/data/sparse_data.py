...
def sparse_data_split(args, data):
    ...
    if args.dataset.valid_ratio == 0:
        train_csr = csr_matrix(
            (
                data["train"]["rating"],
                (data["train"]["user_id"], data["train"]["isbn"]),
            ),
        ...
        train_csr = csr_matrix(
            (train_df["rating"], (train_df["user_id"], train_df["isbn"])),
            shape=(num_users, num_items),
        )
        valid_csr = csr_matrix(
            (valid_df["rating"], (valid_df["user_id"], valid_df["isbn"])),
            shape=(num_users, num_items),
        )
        data["train_csr"] = train_csr
    ...
    test_csr = csr_matrix(
        (np.ones(len(data["test"])), (data["test"]["user_id"], data["test"]["isbn"])),
        shape=(num_users, num_items),
    )
...
class SparseDataset(Dataset):
    ...
    ):  # test_data라고 돼있지만 valid data도 이걸 씁니다
    ...
    def __getitem__(self, idx):
        ...
        if self.test_data is not None:
            ...
            return (
                torch.from_numpy(train_matrix).float(),
...
def sparse_data_loader(args, data):
    ...
    valid_dataset = (
        SparseDataset(data["train_csr"], data["valid_csr"])
        if args.dataset.valid_ratio != 0
        else None
    )
