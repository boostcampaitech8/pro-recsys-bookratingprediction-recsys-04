...
def process_text_data(ratings, users, books, tokenizer, model, vector_create=False):
    ...
    if vector_create:
        ...
        for books_read in tqdm(users_["books_read"]):
            ...
            read_books = read_books.sort_values("review_count", ascending=False).head(
                5
...
class Text_Dataset(Dataset):
    def __init__(
        self, user_book_vector, user_summary_vector, book_summary_vector, rating=None
    ):
        """
        Parameters
        ----------
...
def text_data_load(args):
    ...
    # 1. Merge가 완료된 train_df, test_df 생성
    ...
    # [수정 1] all_df는 'merge가 끝난' 데이터프레임을 합쳐야 모든 컬럼(features)이 포함됩니다.
    ...
    for col in sparse_cols:
        ...
        # [수정 2] cat.codes 대신 map(label2idx) 사용 + fillna 처리
